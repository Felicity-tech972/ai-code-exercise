...
class TestTaskPrioritization(unittest.TestCase):
    def test_overdue_boost_logic(self):
        # Create a task that was due yesterday
        yesterday = datetime.now() - timedelta(days=1)
        task = Task("Overdue Task", priority=TaskPriority.MEDIUM)
        task.due_date = yesterday
        
        # MEDIUM (20) + OVERDUE (35) = 55
        score = calculate_task_score(task)
        self.assertEqual(score, 55)

    def test_completed_task_penalty(self):
        task = Task("Done Task", priority=TaskPriority.URGENT)
        task.status = "done"
        
        # URGENT (60) - PENALTY (50) = 10
        score = calculate_task_score(task)
        self.assertEqual(score, 10)

if hasattr(task, 'assignee') and task.assignee == "current_user":
    score += 12

def test_full_workflow_integration(self):
    manager = TaskManager() # Real storage
    task_id = manager.create_task("Integration Test", priority_value=4)
    
    # Verify the task actually reached the storage
    stats = manager.get_statistics()
    self.assertEqual(stats['total'], 1)
    self.assertEqual(stats['by_priority']['URGENT'], 1)
 
 **Reflection**
i learned from this code that in addition to identifing
problembs TDDs entails designing the code in  a way
its testible.